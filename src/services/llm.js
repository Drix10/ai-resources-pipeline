...
Return ONLY the complete raw text ready to post on LinkedIn.`;
  ...
  validateCommentReply(replyText, postText = "") {
    ...
    if (/\bwhat settles? it\b|\bthe interesting part\b|\b(really|very|so|quite|pretty)\s+interesting\b|\bthis highlights?\b|\bis equivalent to\b|\bis basically\b|\bis the same as\b|\bmaps? (neatly |directly )?to\b|\bboils down to\b|\bmak(?:e|es|ing) it easier to\b|\bgreat breakdown\b|\bgood breakdown\b|\bimportant distinction\b|\breally shows?\b|\bkey takeaways?\b|\bspeaks volumes\b|\bsays a lot\b|\bsheds light\b|\bgame[- ]changer\b/i.test(reply)) {
...
- Before generating, name the exact source facts AND relationships supporting the contribution. If it needs outside knowledge, a new mechanism, failure mode, remedy, causal link, or connecting facts the post never connects: SKIP. A technically correct statement is still invalid when the post doesn't support it.
- FALLBACK (only when no safe contribution exists): one short sentence under 100 chars endorsing the post's specific point with its own nouns ("The resume-vs-circuit-understanding gap is the whole problem."). No new claims, no praise openers, no questions, no numbers. Vague but TRUE beats specific but invented - never force a contribution.
...
- restating the post, verbatim 6-word runs lifted from it, wedged author name
...
- a second sentence verifiable entirely from the post
...
Return ONLY the comment text, or exactly SKIP.`;
  ...
  async criticFeedComment(draft, post) {
    try {
      const prompt = `You are a strict critic of LinkedIn replies. SOURCE POST: """${String(post).slice(0, 1200)}""" PROPOSED REPLY: """${String(draft).slice(0, 500)}""" FAIL the reply if ANY holds: (1) restates the post without adding an observation, implication, correction, extension, or tradeoff; (2) generic praise or agreement; (3) author name used unnaturally; (4) any claim, comparison, number, or causal link NOT supported by the post; (5) vague comparative or filler phrasing ("settles it", "highlights", "breakdown", "testament", "interesting part"); (6) makes it about the commenter, not the technical subject; (7) the author name is wedged or used awkwardly (mid-sentence comma sandwich, tacked-on tag with no grammatical role). The reply's main point must NOT be verifiable entirely from the post - if every sentence restates or rewords post content, FAIL even when well written. (8) ENTITY/EVENT ATTRIBUTION: list every person, device, model, event, number, date in the reply - each must exist in the post AND stay attached to its own event (one exploit's time window on another incident's device = FAIL). (9) NO NEW EVIDENCE: interpretations allowed (implication, tradeoff, failure mode, distinction); new statistics, events, entities, benchmarks, causal claims, or technical facts = FAIL. (10) ALREADY-STATED DISTINCTION: if the post itself draws the distinction the reply offers, FAIL. (11) SHORT ENDORSEMENT: a single sentence under ~100 chars that endorses the post's specific point using its own nouns, with zero new claims, praise, questions, or numbers, PASSES. (12) UNSUPPORTED RELATIONSHIP: never infer causation, correlation, intent, frequency, dependency, or significance merely because two facts share a post. Each asserted relationship must be stated in the source or follow by direct logic - "69 checkpoints indicate manual intervention" and "the sponsored sandbox made the workshop possible" FAIL even though every fact is real. (13) RELATIONSHIP PRESERVATION: preserve the subject/object edges the source states. If the post says A causes B, the reply must not rewrite it as C causes B or C affects B. Speculative hedges ("likely impacts", "may affect", "could explain", "probably caused", "appears to drive") FAIL when the source never establishes the relationship. Strengthening a stated possibility into certainty FAILS. (14) DERIVED SOLUTION: remedies, mitigations, optimizations, recommendations, or design changes that would merely address the post's problem = FAIL. (15) DERIVED FAILURE MODE: hypothetical failure modes plausible for the described system but absent from the post = FAIL. (16) DOMAIN IMPORT: evaluation methods, architectures, mechanisms, or concepts from adjacent domains, not present or directly implied = FAIL. (17) SEMANTIC RESTATEMENT: FAIL when the reply merely compresses, paraphrases, generalizes, or renames relationships already stated by the source - a change in wording, abstraction level, or terminology is not a contribution ("humans repeatedly teach the machine" -> "human effort is required to re-explain it" is the SAME proposition). Reply with exactly one line: PASS or FAIL: <one-line reason>.`;
